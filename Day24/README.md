...
Rather than risk getting shocked while tinkering with the live system, you write down all of the gate connections and initial wire values (your puzzle input) so you can consider them in relative safety.<br>
<b>For example:</b>
```
x00: 1
x01: 1
...
In the smaller example, simulating these gates eventually causes 0 to appear on wire `z00`, 0 to appear on wire `z01`, and 1 to appear on wire `z02`.
...
In the smaller example, the three output bits form the binary number 100 which is equal to the decimal number 4.

After waiting for values on all wires starting with z in the larger example, the wires in this system have the following values:
```
bfw: 1
bqk: 1
djm: 1
